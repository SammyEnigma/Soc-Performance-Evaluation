...
#define XML_SPLIT "[4123:4857:9873:3822]"

class StringUtil
{
public:
    static QStringList getStrMids(const QString& text, QString l, QString r)
    {
        QStringList ans;
        int posl = 0, posr = 0;
        while (posl != -1 && posr != -1)
    ...
    static QString getStrMid(const QString& text, QString l, QString r)
    {
    ...
    static QString fnEncode(QString text)
    {
        QString cs[] = {"\\", "/", ":", "*", "?", "\"", "<", ">", "|", "\'", "\n", "\t"};
        for (int i = 0; i < 12; i++)
    ...
    static QString fnDecode(QString text)
    {
        QString cs[] = {"\\", "/", ":", "*", "?", "\"", "<", ">", "|", "\'", "\t", "\n"};
        for (int i = 0; i < 12; i++)
    ...
    static bool canRegExp(const QString& str, const QString& pat)
    {
        return QRegExp(pat).indexIn(str) > -1;
    }

    static QString getXml(const QString& str, const QString& pat)
    {
    ...
    static int getXmlInt(const QString &str, const QString &pat)
    {
    ...
    static QStringList getXmls(const QString& str, const QString& pat)
    {
    ...
    static QString makeXml(QString str, const QString& pat)
    {
        str = str.replace("</"+pat+">", XML_SPLIT);
        return QString("<%1>%2</%3>").arg(pat).arg(str).arg(pat);
    }

    static QString makeXml(int i, const QString& pat)
    {
        return QString("<%1>%2</%3>").arg(pat).arg(i).arg(pat);
    }

    static QString makeXml(qint64 i/*专门针对时间戳*/, const QString& pat)
    {
        return QString("<%1>%2</%3>").arg(pat).arg(i).arg(pat);
    }

    static bool isBlankChar(QString c)
    {
        return (c == " " || c == "\n" || c == "　" || c == "\t" || c == "\r");
    }

    static bool isBlankChar2(QString c)
    {
        return (c == " " || c == "　" || c == "\t");
    }
    ...
    static QString repeatString(QString s, int i)
    {
        QString ans = "";
        while (i--)
            ans += s;
    ...
    static QString removeBlank(QString s, bool start, bool end)
    {
        int len = s.length();
        if (start)
        {
     ...
     * @return      删首尾空后的文本
     */
    static QString simplifyChapter(QString text)
    {
    ...
    static QString urlEncode(QString s)
    {
        return QUrl::toPercentEncoding(s);
    }

    static QString urlDecode(QString s)
    {
        return QUrl::fromPercentEncoding(s.toUtf8());
    }

    static bool canBeNickname(QString s)
    {
        QRegExp re("^([\\w@_]|[^\\x00-\\xff]){2,20}$");
        return re.exactMatch(s);
    }

    static QString ArabToCN(int num)
    {
    ...
    static int CNToArab(QString text)
    {
        int len = text.length();
        qint64 val = 0;
        int find_pos;
